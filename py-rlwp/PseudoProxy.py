...
class RequestFactory(ServerFactory):
    ...
    def req(self, url):
        ...
        if time.time() >= 1 + self.last_token_time:
            tokens = int(self.tokens_per_10 * (time.time()/10))
            tokens = 10 if (tokens > 10) else tokens
        ...
        if self.curr_tokens_10 <= 1:
            reactor.callLater(self.req)
        try:
            self.curr_tokens_10 -= 1
            resp = self.service.make_request(url) # make request to Riot
            code = self.service.get_response_codes(resp) # get response code
            if 'X-Rate-Limit-Count' in resp.headers:
                self.rate_limit_calls_per_10, self.rate_limit_calls_per_600 = self.service.parse_rate_limit(resp) # record rate liimts
            # tail call recursion?
            if code == 'Retry':
                return self.req(url)
            else:
